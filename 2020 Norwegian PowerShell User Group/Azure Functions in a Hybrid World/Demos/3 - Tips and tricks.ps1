...
Get-AzFunctionApp -Name psfunctiondemo -ResourceGroupName npug-rg | fl *identity*
Get-AzFunctionApp -Name psfunctiondemo -ResourceGroupName npug-rg | Update-AzFunctionApp -IdentityType SystemAssigned

# Grant the Managed Identity access to the secrets inside the vault

$AzADServicePrincipal = Get-AzADServicePrincipal -ObjectId (Get-AzFunctionApp -Name psfunctiondemo -ResourceGroupName npug-rg).IdentityPrincipalId
...
@Microsoft.KeyVault(SecretUri=https://npug.vault.azure.net/secrets/RBKAutomation/4a814b653aa545b4a085e19010709be1)
...
Set-AzKeyVaultSecret -VaultName npug-2 -Name RBKAutomationRSAKey -SecretValue $Secret
$Secret = (Get-AzKeyVaultSecret -VaultName npug-2 -Name RBKAutomationRSAKey | Select-Object -ExpandProperty SecretValueText)
